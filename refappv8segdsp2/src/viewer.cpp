// ngd_app/viewer.cpp — live OpenCV 3D map viewer (no Pangolin/VTK).
//
...
// See viewer.h for the Pangolin->OpenCV mapping rationale.
...
/* ---------------- viewer state ---------------- */
/* The default view replicates the original Pangolin Map viewer (Viewer.cc):
 * eye at (vpX,vpY,vpZ) looking at the origin with up=(0,-1,0), focal=vpF.
 * Same px/m scale as the original, so camera motion is equally prominent. */
                         ...
                         float vpX, float vpY, float vpZ, float vpF)
{
    ...
    v->target[0]=0.0f; v->target[1]=0.0f; v->target[2]=0.0f;   /* look at origin */
    v->up[0]=0.0f;    v->up[1]=-1.0f;   v->up[2]=0.0f;          /* (0,-1,0) like Pangolin */
    v->focal = (vpF>1.0f)?vpF:500.0f;
    v->kf_size   = 0.05f;   /* Viewer.KeyFrameSize (TUM3.yaml) */
    v->cam_size  = 0.08f;   /* Viewer.CameraSize  (TUM3.yaml) */
    v->cap_traj  = 4096;
...
/* Compute the viewer-camera projection (rvec, tvec, K) once per frame from the
 * stored eye/target/up. OpenCV camera convention: +x right, +y DOWN, +z forward
 * => rows = [r; -u; f]. Focal is Viewer.ViewpointF (matches the original). */
static void view_setup(const ngd_app_viewer *v, cv::Mat &rvec, cv::Mat &tvec, cv::Mat &K)
{
    ...
    ngd_vec3 f = v3normed(v3sub(target, eye));   /* forward (eye->target) */
    ngd_vec3 r = v3normed(v3cross(f, up));        /* right  */
    ngd_vec3 u = v3cross(r, f);                    /* true up */
...
/* Project an array of world points (ngd_vec3) -> 2D canvas pixels. */
...
/* Draw a Pangolin-MapDrawer-style wireframe camera frustum at pose Twc.
 * Frustum (camera frame): apex (0,0,0) + 4 corners (±w,±h,z), h=w*0.75, z=w*0.6.
 * 8 lines: apex->each corner (4) + image rectangle (4). */
                         ...
                         ngd_se3 Twc, float w, const cv::Scalar &color, int thick)
{
    ...
    if (p.size()!=5) return;
    /* apex(0)->corners(1..4), then rectangle edges 1-2, 3-4, 4-1, 3-2 */
...
/* Rodrigues rotation of vector v around unit axis k by angle th. */
...
/* mouse: orbit eye around target (drag) + dolly toward/away (wheel). */
static void on_mouse(int event, int x, int y, int flags, void *userdata)
{
    ...
    if (event==cv::EVENT_LBUTTONDOWN){ v->dragging=1; v->drag_last_x=x; v->drag_last_y=y; }
        ...
        float da=(float)(x - v->drag_last_x)*0.01f;   /* yaw (around true up u) */
        float de=(float)(v->drag_last_y - y)*0.01f;   /* pitch (around right r) */
        ngd_vec3 u=v3cross(r,fwd);
        ngd_vec3 nf=v3rot(fwd,u,-da);
        nf=v3rot(nf,r,-de);
        /* keep fwd away from up so the right axis doesn't degenerate */
                         ...
                         int state, int nInliers, uint64_t frame_num)
{
    ...
    if (cur) traj_push(v, ngd_frame_get_camera_center(cur));

    if (!v->win_created){
        cv::namedWindow("NGD refapp: 3D Map", cv::WINDOW_NORMAL);
        cv::resizeWindow("NGD refapp: 3D Map", v->W3d, v->H3d);
        cv::setMouseCallback("NGD refapp: 3D Map", on_mouse, v);
        v->win_created=1;
    }
    ...
    if (!cloud.empty()){
        ...
        for (size_t i=0;i<cloud.size();++i){
            float hc=v3dot(cloud[i],upv);   /* height along the view-up axis */
    ...
    /* ---- covisibility graph (Pangolin DrawKeyFrames bDrawGraph): green lines
     *      between KF centres for neighbours with weight>=100, mnId<j dedup. ---- */
    if (kfc.size()>=2){
        ...
        for (size_t i=0;i<kfs.size();++i){
            ...
            for (int j=0;j<a->nConn;++j){
                ...
                if (!b || b->mnId <= a->mnId) continue;   /* draw each edge once */
                /* find b's index in kfs[] (linear; nConn small, nKF small) */
                int bi=-1;
    ...
    /* ---- keyframe frustums (Pangolin DrawKeyFrames bDrawKF): blue, init KF red ---- */
    {
        const cv::Scalar kf_col(255,0,0);     /* blue (BGR) */
        const cv::Scalar init_col(0,0,255);   /* red: first KF in map */
        uint64_t init_id = map?map->mnInitKFid:(uint64_t)-1;
        for (size_t i=0;i<kfs.size();++i){
            const ngd_keyframe *kf=kfs[i];
            int is_init = (kf->mnId==init_id);
            draw_frustum(canvas, rvec,tvec,K, ngd_se3_inverse(kf->pose),   /* Twc */
                         v->kf_size, is_init?init_col:kf_col, is_init?2:1);
        }
    }

    /* ---- continuous camera trajectory (dim green polyline; current tip bright) ---- */
    if (v->n_traj>=2){
    ...
    /* ---- current camera frustum (Pangolin DrawCurrentCamera): green ---- */
    if (cur)
        draw_frustum(canvas, rvec,tvec,K, ngd_se3_inverse(cur->pose), v->cam_size, cv::Scalar(0,255,0), 2);

    /* ---- HUD ---- */
    char buf[192];
    std::snprintf(buf,sizeof(buf),"frame %llu  state=%d  inliers=%d  KFs=%d  MPs=%d  traj=%d",
        (unsigned long long)frame_num, state, nInliers, (int)kfs.size(),
    ...
    cv::imshow("NGD refapp: 3D Map", canvas);

    /* ---- 2D frame overlay ---- */
    if (!imRGB.empty() && cur && cur->keys){
        cv::Mat ov;
        if (imRGB.channels()==3) ov=imRGB.clone();
        else cv::cvtColor(imRGB, ov, cv::COLOR_GRAY2BGR);

        if (mask && maskW>0 && maskH>0 && maskW<=ov.cols && maskH<=ov.rows){
            cv::Mat maskMat(maskH, maskW, CV_8U, (void *)mask, (size_t)maskW);
        ...
        for (int i=0;i<cur->N;++i){
            const ngd_keypoint &k=cur->keys[i];
            cv::Point pt((int)(k.x+0.5f),(int)(k.y+0.5f));
            void *mp = cur->mvpMapPoints ? (void *)cur->mvpMapPoints[i] : NULL;
            int outlier = cur->mvbOutlier ? cur->mvbOutlier[i] : 0;
            cv::Scalar col; int rad;
            if (mp && !outlier){ col=cv::Scalar(0,255,0);  rad=3; }   /* inlier match */
            else if (mp && outlier){ col=cv::Scalar(0,0,255); rad=2; }/* outlier */
            else { col=cv::Scalar(170,170,170); rad=1; }              /* unmatched */
            cv::circle(ov, pt, rad, col, -1, cv::LINE_AA);
        }
        cv::imshow("NGD refapp: Frame", ov);
    }

    int key = cv::waitKey(1) & 0xff;
    if (key=='r'||key=='R'){   /* reset to the YAML default viewpoint */
