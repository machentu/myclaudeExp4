...
#endif
#ifdef VP6_EXTRACT
#include "dsp/dsp_orb_vp6.h"       // VP6_EXTRACT: tile-manager VP6+IVP drivers (cstub on host)
#include "dsp/orb_tile_manager.h"  //         with BRIEF_INT_VP6: fixed-point LUT rBRIEF (approximate)
#endif
                                ...
                                double timestamp)
{
    ...
    if (mbNeedKF) {
...
#ifdef USE_DSP_ORB
        {
             ...
             * VP6_EXTRACT swaps in dsp_orb_vp6_extract (persistent bank-pool
             * drivers; on the DSP build BRIEF_INT_VP6 selects the fixed-point
             * LUT rBRIEF — approximate descriptors by design). */
#ifdef VP6_EXTRACT
            static xvTileManager s_tm;
            static int s_tm_ok = 0;
            int Nraw = -1;
            if (s_tm_ok || orb_tile_manager_init(&s_tm) == 0) {
                s_tm_ok = 1;
                Nraw = dsp_orb_vp6_extract(&s_tm, ex, gray, W, H, W, kps, MAXK, desc);
            }
#else
            int Nraw = dsp_orb_extract(ex, gray, W, H, W, kps, MAXK, desc);
#endif
            /* opt-in golden dump for the real-DSP RPC bring-up (dspcalling/):
             * NGD_DUMP_ORB_GOLDEN=<dir> saves the first 8 extractor calls
             * (input gray + raw keypoints + descriptors) in the exact file
             * format dspcalling_orb_example writes/compares. Raw output,
             * before the mask filter below. */
            {
                static int s_dump_n = 0;
                static const char *s_dump_dir = getenv("NGD_DUMP_ORB_GOLDEN");
                if (s_dump_dir && s_dump_n < 8 && Nraw >= 0) {
                    char path[512];
                    FILE *f;
                    snprintf(path, sizeof(path), "%s/f%03d_gray.bin",
                             s_dump_dir, s_dump_n);
                    if ((f = fopen(path, "wb")) != NULL) {
                        fwrite(gray, 1, (size_t)W * H, f); fclose(f);
                    }
                    snprintf(path, sizeof(path), "%s/f%03d.kps",
                             s_dump_dir, s_dump_n);
                    if ((f = fopen(path, "wb")) != NULL) {
                        fwrite(&Nraw, sizeof(Nraw), 1, f);
                        fwrite(kps, sizeof(kps[0]), (size_t)Nraw, f);
                        fclose(f);
                    }
                    snprintf(path, sizeof(path), "%s/f%03d.desc",
                             s_dump_dir, s_dump_n);
                    if ((f = fopen(path, "wb")) != NULL) {
                        fwrite(desc, 1, (size_t)Nraw * 32, f); fclose(f);
                    }
                    ++s_dump_n;
                }
            }
