 ...
 * Non-static: the IVP kernel in ic_angle_vp6.c reuses it on the same integers. */
...
float ic_fast_atan2(float y, float x)
{
...
float ic_angle_full_opt(const uint8_t *img, int w, int h, int stride,
                        int cx, int cy, const int *umax)
{
    /* In the SLAM pipeline every keypoint is >= minBorder(16) from the frame
     * edge, so the +/-15 patch (and thus every clamp in ic_pix_clamp) is
     * identity. Detect that once and run row-pointer reads with no per-sample
     * clamp branches; the arithmetic is statement-for-statement the same as
     * ic_angle_kernel, so the result is bit-identical. */
    if (cx - IC_ANGLE_HALF_PATCH >= 0 && cx + IC_ANGLE_HALF_PATCH < w &&
        cy - IC_ANGLE_HALF_PATCH >= 0 && cy + IC_ANGLE_HALF_PATCH < h) {
        const uint8_t *pc_mid = img + (size_t)cy * stride + cx;
        int m_01 = 0, m_10 = 0;
        ...
        for (int v = 1; v <= IC_ANGLE_HALF_PATCH; ++v) {
            const uint8_t *pc_p = img + (size_t)(cy + v) * stride + cx;
            const uint8_t *pc_m = img + (size_t)(cy - v) * stride + cx;
            int v_sum = 0;
            int d = umax[v];
            for (int u = -d; u <= d; ++u) {
                int val_plus  = (int)pc_p[u];
                int val_minus = (int)pc_m[u];
                v_sum += (val_plus - val_minus);
                m_10 += u * (val_plus + val_minus);
            }
            m_01 += v * v_sum;
        }
        return ic_fast_atan2((float)m_01, (float)m_10);
    }
    return ic_angle_full(img, w, h, stride, cx, cy, umax);
}
