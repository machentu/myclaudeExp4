                  ...
                  int cx, int cy, float angle, uint8_t *desc)
{
    ...
    float a = (float)cos(ar), b = (float)sin(ar);   /* cv::computeOrbDescriptor uses cos/sin on double */
...
/* Exact rint/rintf replacement for the rotated-offset floats (|f| <= 19):
 * trunc toward zero, then fix up the residual q = f - trunc(f), which is
 * exactly representable at these magnitudes, so every step is exact and the
 * result matches round-half-to-even under any intermediate precision.
 * Avoids the 32-bit CRT rintf call (a function call per rounding) that
 * profiling showed dominates descriptor time. */
static inline int brief_round_even_inline(float f)
{
    int t = (int)f;                  /* trunc toward zero */
    float q = f - (float)t;
    if (q > 0.5f) return t + 1;
    if (q < -0.5f) return t - 1;
    if (q == 0.5f) return (t & 1) ? t + 1 : t;    /* half -> even neighbour */
    if (q == -0.5f) return (t & 1) ? t - 1 : t;
    return t;
}

void brief_full_opt(const uint8_t *img, int w, int h, int stride,
                    int cx, int cy, float angle, uint8_t *desc)
{
    /* Rotated offsets are bounded by rint(13*sqrt(2)) = 18 <= BRIEF_MARGIN(19)
     * in both row and col, so a keypoint at least 19 px from every frame edge
     * never clamps. Detect that once and run direct reads with no per-sample
     * clamp branches; brief_round_even_inline replaces rint((double)) with an
     * identical result (exact trunc + residual round-half-to-even). Otherwise
     * identical to brief_kernel -> bit-identical descriptors. */
    if (cx - BRIEF_MARGIN >= 0 && cx + BRIEF_MARGIN < w &&
        cy - BRIEF_MARGIN >= 0 && cy + BRIEF_MARGIN < h) {
        float ar = angle * (float)(BRIEF_PI / 180.0);
        ...
        for (int i = 0; i < 32; ++i) {
            ...
            for (int bit = 0; bit < 8; ++bit) {
                ...
                int p1x = brief_bit_pattern_31[pair][0], p1y = brief_bit_pattern_31[pair][1];
                int p2x = brief_bit_pattern_31[pair][2], p2y = brief_bit_pattern_31[pair][3];
                int c1 = brief_round_even_inline(p1x * a - p1y * b), r1 = brief_round_even_inline(p1x * b + p1y * a);
                int c2 = brief_round_even_inline(p2x * a - p2y * b), r2 = brief_round_even_inline(p2x * b + p2y * a);
                int t0 = (int)img[(size_t)(cy + r1) * stride + (cx + c1)];
                int t1 = (int)img[(size_t)(cy + r2) * stride + (cx + c2)];
                val |= (t0 < t1) << bit;
            }
            desc[i] = (uint8_t)val;
        }
    ...
    brief_full(img, w, h, stride, cx, cy, angle, desc);
}
