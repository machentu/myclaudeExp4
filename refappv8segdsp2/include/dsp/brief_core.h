...
/*
 * Accelerated full-image rBRIEF: same output as brief_full, bit-exact.
 * When the rotated patch (+/-18 worst case, bounded by BRIEF_MARGIN 19) is
 * fully in frame, samples directly with no per-sample clamp branches and
 * rintf instead of rint((double)) (exact conversion, same half-to-even rule);
 * otherwise falls back to brief_full.
 */
