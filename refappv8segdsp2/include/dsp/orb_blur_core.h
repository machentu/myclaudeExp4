 ...
 * (7-tap separable, sigma=2, reflect-101 boundary, ufixedpoint16 kernel shift8,
 * (sum+32768)>>16 round). The fixed-point arithmetic is UNCHANGED so the kernel is
 * bit-exact with both cv::GaussianBlur and the refactor C reference.
 *
...
/* 7-tap Gaussian, sigma=2, ufixedpoint16 shift8, sum=256. Extracted from
 * OpenCV 4.12 getGaussianKernelFixedPoint_ED(7,2,8). Copied verbatim from
 * refactor orb.c ngd_gauss7. */
extern const uint16_t orb_blur_gauss7[7];

/*
 * Separable 7-tap Gaussian blur on one tile (GaussianBlurFixedPoint path).
 *
 ...
 *                        (uint16_t raw sums). tmp_pitch >= dst_w; tmp holds
 *                        region_h rows of width dst_w, i.e. needs
 *                        region_h * tmp_pitch * sizeof(uint16_t) bytes.
 *
 ...
 * own tmp (uint16_t). This is the oracle the VP6 tile path is compared against;
 * it is bit-exact with refactor's ngd_gaussian_blur by construction (same maths).
 * dst may == src.
 */
void orb_blur_full(const uint8_t *src, int w, int h, int stride, uint8_t *dst);

/*
 * Accelerated full-image blur: same contract and bit-exact output as
 * orb_blur_full. Interior pixels use the symmetric-pair form
 * (S = g0*(v- + v+) + ... + g3*v, g symmetric — the same exact integer sum,
 * 4 multiplies instead of 7) over row pointers with no reflect branch;
 * border pixels (window crossing the frame edge, 3 cols/rows each side)
 * keep the original direct reflect-101 path. The multiply halving is the
 * win that carries to IVP lanes on DSP.
 */
