...
/*
 * Bit-exact port of cv::fastAtan2 (refactor ngd_fast_atan2): degrees in
 * [0,360). Shared by ic_angle_kernel and the IVP kernel in ic_angle_vp6.c,
 * which feed it the same integer m_01/m_10 moments.
 */
...
/*
 * Accelerated full-image IC angle: same output as ic_angle_full, bit-exact.
 * When the +/-15 patch is fully in frame (always true for SLAM keypoints,
 * which sit >= minBorder 16 from the edge), reads via row pointers with no
 * per-sample clamp; otherwise falls back to ic_angle_full.
 */
