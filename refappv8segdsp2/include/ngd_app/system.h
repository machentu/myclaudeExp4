 ...
 *
 * Phase v8seg: NGD dynamic filtering with END-TO-END YOLOv8-seg mask.
 * Per frame (GrabImageRGBD-equivalent):
 *   cvtColor(imRGB) -> gray
 *   Yolov8SegDetector.detect(imRGB, imDepthMeters) -> mask (person mask, direct;
 *                            NO LK PredictCurrentMask propagation in this variant)
...
class Yolov8SegDetector;   // forward decl (ngd_app/yolov8_seg.h, global namespace)
...
struct ngd_app_system {
    ...
    /* per-frame pose covariance for NEES (PnP approximation, post-hoc: does not
     * affect tracking). relCov: nRel*36 row-major 6x6, camera-frame [rvec;tvec]
     * perturbation of T_cw (Sigma = (J^T Omega J)^-1, J = cv::projectPoints
     * jacobian over PnP inliers). relCovValid: 1 if >=6 inliers and invertible. */
    float             *relCov;
    int               *relCovValid;

    /* NGD mask state (refappv8seg: end-to-end YOLOv8-seg, no LK propagation). */
    Yolov8SegDetector     *yolo;     /* owned; NULL if model unavailable */
    uint8_t           *gray_buf;       /* imgW*imgH, cvtColor output */
    uint8_t           *last_gray;      /* previous frame gray (OF-pose LK source) */
    uint8_t           *last_mask;      /* previous frame mask (KF-branch dyna-extractor gate) */
    uint8_t           *mask_curr;      /* YOLOv8-seg mask applied to the current frame */
    int                has_last;       /* reserved (no longer gates LK; kept for minimal diff) */
    uint64_t           frame_id;
...
/* Load vocab + settings, build map/kfdb/lm/loopclosing/tracking + Yolov8SegDetector.
...
/* Write per-frame pose covariance (PoseCovariance.txt) for NEES evaluation.
 * Format: "ts valid c00 c01 ... c55" (row-major 6x6, camera-frame [rvec;tvec]). */
