 ...
 *   (3) ic_angle_vp6   - the VP6 per-keypoint patch-DMA driver running the IVP
 *                       kernel (clamped gather offsets + per-row disk masking +
 *                       per-column accumulators + scalar moment fold).
 *
 * (1)==(2) proves ic_angle_core matches refactor. (2)==(3) proves the IVP/VP6
 * path is bit-exact with the PC reference. Together: VP6 == refactor, per keypoint,
