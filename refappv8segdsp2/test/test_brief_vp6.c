 ...
 *                    with the IVP intrinsic kernel (gather pixel access),
 *                    built via cstub.
 *
 ...
 *   (3) brief_vp6   - the VP6 per-keypoint patch-DMA driver running the IVP
 *                     kernel (scalar offset precompute + GATHER/LTU/MOVUT
 *                     vector compare + scalar bit pack).
 *
 * (1)==(2) proves brief_core matches refactor. (2)==(3) proves the IVP/VP6
 * path is bit-exact with the PC reference. Together: VP6 == refactor, per
 * keypoint, all 32 descriptor bytes identical. Covers gradient/random/checker
 * images, keypoints in the interior AND near the frame edge (exercises the
 * edge-clamp), random angles [0,360) plus fixed angles (0/45/90/180), and
 * several counts.
 */
#include <stdio.h>
#include <stdlib.h>
...
static void ref_compute_descriptor(const uint8_t *img, int w, int h,
    ...
    float a = (float)cos(ar), b = (float)sin(ar);   /* cos/sin on double, matching cvRound */
    for (int i = 0; i < 32; ++i) {
        int val = 0;
