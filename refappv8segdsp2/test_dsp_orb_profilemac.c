...
#define _LOCAL_DRAM0_  __attribute__((aligned(64), section(".dram0.data")))
#define _LOCAL_DRAM1_  __attribute__((aligned(64), section(".dram1.data")))
...
#define PROFILE
    ...
    int n2 = dsp_orb_vp6_extract(&tm, &ex2, gray, IW, IH, IW, kps2, maxk, desc2);
//    int n2 = dsp_orb_extract(&ex2, gray, IW, IH, IW, kps2, maxk, desc2);

#ifdef PROFILE
    TIME_STAMP(stop);
    int32_t cycles_dsp = stop - start;
    printf("cycle: %d\n", cycles_dsp);
#endif
