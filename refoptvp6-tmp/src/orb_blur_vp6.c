                         ...
                         const OrbBlurVp6Config *cfg)
{
     ...
     * Holds region_h (<= MSH) rows of width TW. On the DSP this is local SRAM;
     * under cstub it is host memory (the portable kernel does scalar byte
     * access, so alignment is not required for correctness). */
    uint8_t *tmp = (uint8_t *)malloc((size_t)MSH * TW);
    if (!tmp) {
