...
//
// NGD OF mode: three LK/PnP backends, selected by NGD_LK_BACKEND env:
//   unset / "opencv" → OpenCV LK + cv::solvePnPRansac (default)
//   "u8"             → pure-C uint8 pyramid LK + EPnP (13.4cm ATE)
//   "float"          → pure-C float pyramid LK + EPnP
// NGD_PNP=mlpnp switches EPnP→MLPnP (pure-C backends only).
...
/* NGD_OF_LK_C - OF tracking LK backend selector (code-level define).
 *   1 = pure-C float LK + OpenCV solvePnPRansac (1.58cm, ~35% slower; drops the
 *       OpenCV LK dependency) — DEFAULT. Pure-C LK matches OpenCV accuracy.
 *   0 = OpenCV LK (1.61cm/60s).
 * NOTE: pure-C EPnP/MLPnP solvers lose ~0.6cm (2.2cm), so PnP stays solvePnPRansac.
 * Flip this default, or compile with -DNGD_OF_LK_C=0. See BENCHMARK §10.5. */
#ifndef NGD_OF_LK_C
#define NGD_OF_LK_C 1
#endif
...
#endif
#ifndef NGD_LK_OBS3
#define NGD_LK_OBS3 0        /* 1 = Observations>=3 gate (original SearchByOpticalFlow) */
#endif
#ifndef NGD_MIN_EIG
#define NGD_MIN_EIG 0.0f     /* LK min eigenvalue; 0 = disabled */
#endif
#ifndef NGD_LK_FB
#define NGD_LK_FB 1.0f       /* pure-C LK FB consistency threshold (px); 0 = off */
#endif
#ifndef NGD_PNP
                                                ...
                                                const uint8_t *mask)
{
     ...
     * it left too few LK candidates -> c2 over-fired -> ORB ~every frame (99%).
     * Set NGD_LK_OBS3=1 to restore the original obs>=3 gate (A/B vs original). */
     ...
     * Three LK backends, selected by NGD_LK_BACKEND env:
     *   unset / "opencv" → OpenCV     + solvePnPRansac (default, reference quality)
     *   "u8"             → pure-C u8  + EPnP
     *   "float"          → pure-C f32 + EPnP           */
    /* NGD_LK_BACKEND (define): 0=opencv, 1=u8, 2=float — used in the #else below */
#if NGD_OF_LK_C
    /* Compile-time: pure-C float LK (define NGD_OF_LK_C). The pure-C LK itself
     * matches OpenCV accuracy (float 1.58 vs 1.61cm fr3_walk); the accuracy gap
     * people saw is the pure-C EPnP/MLPnP solver (2.2cm), NOT the LK. So this
     * define pairs pure-C LK with OpenCV solvePnPRansac (forced below) to keep
     * accuracy. Cost: ~35% slower (82 vs 60s, no SIMD). See BENCHMARK §10.5. */
    int use_opencv = 0;
    int use_float  = 1;   /* float LK 1.58cm beats u8 1.66cm */
#else
     ...
     * Pure-C backends → EPnP (or MLPnP via NGD_PNP=mlpnp)             */
